...
//        正如你所见的，你创建了一个用于添加实体（Entity）的模式（Schema）对象。两个参数分别代表：数据库版本号与自动生成代码的包路径。
        Schema schema = new Schema(1, "package_bean");
//        当然，如果你愿意，你也可以分别指定生成的 Bean 与 DAO 类所在的目录，只要如下所示：
//        Schema schema = new Schema(1, "com.ihesen.think.db.bean");
//        schema.setDefaultJavaPackageDao("com.ihesen.think.db.dao");
...
//        new DaoGenerator().generateAll(schema, "C:\\Users\\ihesen\\Desktop\\GreenDaoForAndroidStudio\\DaoExample\\src\\main\\java-gen");
        new DaoGenerator().generateAll(schema, "app/src/main/java/com/ihesen/think/utils/db");
    }
...
//        box.addDateProperty("date");
    ...
    private static void addTestTable(Schema schema){
        Entity box = schema.addEntity("TestTable");
        box.addIdProperty();
        box.addIntProperty("testInt");
    }
}
